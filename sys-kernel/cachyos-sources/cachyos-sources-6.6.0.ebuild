...
detect_version
# COMMIT of CachyOS/linux-cachyos: 249a49f923df80f21e0f713ec727034d4373f6ec
# This is the commit hash for Cachyos/kernel-patches
EGIT_COMMIT="208aceb488c36fd075bd8355e106ebed84e7602f"
...
LICENSE="GPL-3"
KEYWORDS="~amd64"
IUSE="+bore-eevdf eevdf pds bmq tt bore cfs eevdf-rt bore-hardened sched-ext +bcachefs +lrng"
REQUIRED_USE="^^ ( bore-eevdf eevdf pds bmq tt bore cfs eevdf-rt bore-hardened sched-ext )"
...
src_prepare() {
	...
	if use eevdf-rt; then
		eapply "${PATCH_DIR}/sched/0001-rt.patch"
		cp "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/config-cachyos-rt" .config
	fi

	if use bore-hardened; then
		eapply "${PATCH_DIR}/sched/0001-bore-cachy.patch"
		eapply "${PATCH_DIR}/misc/0001-hardened.patch"
		cp "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/config-cachyos-hardened" .config
	fi

	if use sched-ext; then
		eapply "${PATCH_DIR}/sched/0001-bore-cachy.patch"
		eapply "${PATCH_DIR}/sched/0001-sched-ext.patch"
		# sched-ext currently doesn't have its own config file, use config-cachyos
		cp "${FILESDIR}/${KV_MAJOR}.${KV_MINOR}/config-cachyos" .config
	fi
	...
	use eevdf-rt && scripts/config -e PREEMPT_COUNT -e PREEMPTION \
	...
	# Set tick rate
	(use tt || use eevdf-rt || use pds || use bmq) && scripts/config -d HZ_300 \
	...
	# Set tick type
	use eevdf-rt && scripts/config -d HZ_PERIODIC \
		-d NO_HZ_FULL \
		-e NO_HZ_IDLE \
		-e NO_HZ \
		-e NO_HZ_COMMON
